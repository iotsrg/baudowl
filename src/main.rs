...
#[command(
    author,
    version,
    about = "BaudHound - The Ultimate Serial Port Detective",
    long_about = None,
    after_help = "Examples:\n  baudhound -p /dev/ttyACM0 --highspeed\n  baudhound --turbo -q\n  baudhound -n myconfig --auto"
)]
struct Args {
...
impl BaudHound {
    ...
    fn save_minicom_config(&self, baudrate: u32, name: &str) -> Result<(), Box<dyn std::error::Error>> {
        let config = format!(
            "########################################################################\n\
            # Minicom configuration file - generated by BaudHound\n\
            pu port             {}\n\
            pu baudrate         {}\n\
            pu bits             8\n\
            pu parity           N\n\
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    let mut hound = BaudHound::new(args);
    hound.run()
}
